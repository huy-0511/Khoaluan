<?php
    include_once '../lib/sessionkh.php';
    Session::checkSession();
    include_once '../classes/quanlitaikhoan.php';
?>
        ...
        <div class="container">
            ...
            <div class="table-responsive">
                ...
                <div class="form-group">
                    ...
                    <label><?php $date = $date = getdate();    echo $date['mday']."/".$date['mon']."/".$date['year']."    ".$date['hours'].":".$date['minutes'].":".$date['seconds']; ?>
                    </label>
                </div> 
                <div class="form-group">
                    <label >Kết quả:</label>
                        ...
                        if ($secureHash == $vnp_SecureHash) {
                            if ($_GET['vnp_ResponseCode'] == '00') {
                                ...
                                $date_time = substr($time, 0, 4) . '-' . substr($time, 4, 2) . '-' . substr($time, 6, 2) . ' ' . substr($time, 8, 2) . ' ' . substr($time, 10, 2) . ' ' . substr($time, 12, 2);
                                 $sodu = $_GET['vnp_Amount']/100;
                            $id = Session::get('khId');
                            $tk = new qltaikhoan();
                            $inserttien = $tk->themtien_taikhoan($sodu, $id);
                    ...
                    <br>
                    <a href="../khachhang/qlttcn.php?idtaikhoan=<?php echo $id ?>">
